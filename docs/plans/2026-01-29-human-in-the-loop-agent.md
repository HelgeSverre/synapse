...
**Architecture:** A decorator (`ApprovingToolRegistry`) wraps the tool executor and intercepts calls to risky tools. Risk is determined by tool attributes. When a risky tool is called, execution pauses, prompts the human for a decision, and either proceeds, rejects with feedback, or modifies arguments before execution.
...
```php
<?php
// examples/approval-agent/ApprovingToolRegistry.php
declare(strict_types=1);
...
Run: `php -l examples/approval-agent/ApprovingToolRegistry.php`
