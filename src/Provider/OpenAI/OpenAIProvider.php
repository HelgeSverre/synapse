...
namespace HelgeSverre\Synapse\Provider\OpenAI;
...
final readonly class OpenAIProvider implements LlmProviderInterface, StreamableProviderInterface
{
    ...
    public function stream(GenerationRequest $request, ?StreamContext $ctx = null): Generator
    {
        ...
        foreach (SseParser::parseStream($response->getBody(), $ctx) as $event) {
            ...
            if (isset($delta['tool_calls']) && is_array($delta['tool_calls'])) {
                foreach ($delta['tool_calls'] as $toolCallChunk) {
                    ...
                    // Accumulate for final ToolCallsReady event
            ...
            // Usage in chunk (with stream_options)
            if (isset($chunk['usage'])) {
                $usage = new UsageInfo(
                    inputTokens: $chunk['usage']['prompt_tokens'] ?? 0,
                    outputTokens: $chunk['usage']['completion_tokens'] ?? 0,
        ...
        // Emit ToolCallsReady if there were tool calls
        if ($toolCallDeltas !== []) {
            $toolCalls = [];
