...
namespace HelgeSverre\Synapse\Provider\Ollama;
...
final readonly class OllamaProvider implements LlmProviderInterface, StreamableProviderInterface
{
    ...
    private function buildRequestBody(GenerationRequest $request): array
    {
        ...
        if (count($request->tools) > 0) {
            $body['tools'] = array_map(
                fn (ToolDefinition $tool): array => $tool->toOpenAIFormat(),
                $request->tools,
            );
    ...
    private function parseResponse(array $response): GenerationResponse
    {
        ...
        if (isset($message['tool_calls']) && is_array($message['tool_calls'])) {
            foreach ($message['tool_calls'] as $i => $toolCall) {
                ...
                if (is_array($rawArgs)) {
                    ...
                    $arguments = json_decode((string) $rawArgs, true) ?? [];
                }
